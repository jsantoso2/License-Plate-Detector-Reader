...
def load_model():
    # YOLO CONFIGURATIONS
    net = cv2.dnn.readNetFromDarknet('backup/yolov3_custom_train.cfg', 'backup/yolov3_custom_train_1800.weights')
...
def inference(net, pipeline, file_type = 'image'):
    ...
    if file_type == 'image':
        cap = cv2.VideoCapture('appworkspace/image.jpg')
        
    # file uploaded must be video
    else:
        cap = cv2.VideoCapture('appworkspace/video.mp4')
        #vid_writer = cv2.VideoWriter('appworkspace/pred.avi', cv2.VideoWriter_fourcc('M','J','P','G'), 30, (round(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),round(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
        vid_writer = cv2.VideoWriter('appworkspace/pred.mp4', cv2.VideoWriter_fourcc(*'MP4V'), 30, (round(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),round(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    ...
    while cv2.waitKey(1) < 0:
        ...
        if file_type == 'image':
            cv2.imwrite('appworkspace/pred.jpg', frame.astype(np.uint8))
            return text_pred
        else:
...
#    if file_type == 'image':
#        return f'<a href="appworkspace/pred.jpg" download="pred.jpg">Download Output Image</a>'
#    else:
#        return f'<a href="appworkspace/pred_outputs.mp4" download="pred_outputs.mp4">Download Output Video</a>'
...
def main():
    ...
    im = Image.open('appworkspace/demo_image.JPG')
    st.image(im, use_column_width=True)
    
    # video demo
    st.subheader('Video Examples:')
    vid_file = open('appworkspace/demo_video.mp4', 'rb')
    ...
    if uploaded_file is not None:
        ...
        if file_type == 'image':
            ...
            cv2.imwrite('appworkspace/image.jpg', image)
            #st.write('after local')
            #st.image('image.jpg')
        else:
            # Write the mp4 video to local
            with open("appworkspace/video.mp4", "wb") as f:
        ...
        if st.button("Make Prediction"):
            ...
            if file_type == 'image':
                image = Image.open('appworkspace/pred.jpg')
                ...
                ff = FFmpeg(inputs={'appworkspace/pred.mp4': None}, outputs={'appworkspace/pred_outputs.mp4': '-c:v h264'})
                ff.run()
                
                video_file = open('appworkspace/pred_outputs.mp4', 'rb')
